...
assert.match(frontend,/btnSyncWhatsAppAll[\s\S]{0,120}Extensão WhatsApp/);assert.match(frontend,/btnUpdateOfficialAnalysis[\s\S]{0,120}Analisar conversa/);assert.match(frontend,/whatsapp:\/\/send\?phone=\$\{number\}/);assert.match(frontend,/window\.location\.assign\(webUrl\)/);assert.match(frontend,/if\(!isWindows\)return window\.open\(webUrl/);
