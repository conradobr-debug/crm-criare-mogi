...
test("captura exige telefone válido e único e não usa semelhança de nome",async()=>{const crm=await readFile(new URL("../index.html",import.meta.url),"utf8"),content=await readFile(new URL("../whatsapp-crm-extension/content-whatsapp.js",import.meta.url),"utf8");assert.match(crm,/requireWhatsAppIdentity\(record\)/);assert.match(crm,/code:"phone_duplicate"/);const start=content.indexOf("function sameCustomer"),end=content.indexOf("function confirmConversationPhone",start);assert.ok(start>=0&&end>start,"o teste precisa examinar apenas a decisão de identidade, não funções posteriores de URL");const sameCustomer=content.slice(start,end);assert.doesNotMatch(sameCustomer,/expectedName|tokens|searchParams/);assert.match(sameCustomer,/phoneIdentityConfirmed/);assert.match(sameCustomer,/confirmedPhone/);});
