(function(){
  ...
  async function onCandidatePendingCreated(candidateId,pending){const {error}=await sb.from(CANDIDATE_TABLE).update({matched_pending_id:pending.id,updated_at:nowISO()}).eq("id",candidateId);if(error)throw error;await refreshCandidateInbox({silent:true});toast("Pendência criada. Agora vincule ou cadastre o cliente para incluir a conversa na análise de assistência.");}
