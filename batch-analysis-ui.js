(function(){
  ...
  function renderImportPreview(){
    ...
    const counts=results.reduce((map,result)=>(map[result.status]=(map[result.status]||0)+1,map),{});const ready=counts.ready_to_import||0;const received=state.validation?.summary?.received??results.length;const unique=state.validation?.summary?.unique??ready;const duplicates=counts.duplicate||0;const conflicts=counts.duplicate_conflict||0;const already=counts.already_imported||0;const imported=counts.imported||0;const individual=results.reduce((sum,result)=>sum+Number(result.action_plan?.individual||0),0);
    ...
    const identityConflicts=counts.conversation_identity_conflict||0;const detail=`Arquivo: ${file} • únicas: ${unique} • duplicadas: ${duplicates} • conflitos: ${conflicts} • vínculos ambíguos: ${identityConflicts} • já importadas: ${already} • gravações: ${state.actualWrites} • recomendações individuais: ${individual}.`;
