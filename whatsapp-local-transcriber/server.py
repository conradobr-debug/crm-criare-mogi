...
def transcribe(payload: dict) -> dict:
    ...
    cache = load_cache()
    if digest in cache:
        return {"text": cache[digest], "sha256": digest, "cached": True}
    binary = executable()
    model = model_path()
    if not binary or not model:
        return {"code": "TRANSCRIBER_NOT_STARTED", "error": "Transcritor local não está iniciado. Configure whisper.cpp e o modelo Whisper."}
    ...
    with tempfile.TemporaryDirectory(prefix="criare-audio-") as folder:
        safe_id = "".join(char if char.isalnum() or char in "-_" else "_" for char in message_id) or "audio"
        source = pathlib.Path(folder) / f"{safe_id}{suffix}"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if size > 24 * 1024 * 1024:
                ...
                return
            result = transcribe(json.loads(self.rfile.read(size).decode("utf-8")))
