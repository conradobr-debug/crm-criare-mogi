...
async function revealMessageContextMenuTrigger(node,diagnostic){
  ...
  const observed=await waitForContextTrigger(node);diagnostic.mutation_detected=observed.mutationDetected;diagnostic.trigger_found=Boolean(observed.trigger);diagnostic.trigger_visible=Boolean(observed.trigger&&visibleElement(observed.trigger));diagnostic.trigger_selector=observed.trigger?CONTEXT_TRIGGER_SELECTOR:"";diagnostic.status=observed.trigger?"seta_detectada":"seta_nao_detectada";if(!observed.trigger)diagnostic.reason="O WhatsApp não exibiu a seta de menu após o hover sintético.";return observed.trigger||null;
}
...
async function recoverAudioEntries(request={}){
  ...
  const inspect=async()=>{
    for(const [position,node] of voiceMessageNodes(main).entries()){
      ...
      const diagnostic=diagnostics.get(key)||{};const parts=prefixParts(prefix);const playerDuration=audioDuration(node);diagnostic.found_in_dom=true;diagnostic.sender=parts.sender||diagnostic.sender;diagnostic.date=parts.date||diagnostic.date;diagnostic.message_time=parts.time||diagnostic.message_time;diagnostic.duration_text=audioDurationText(node)||diagnostic.duration_text;diagnostic.duration_seconds=playerDuration||diagnostic.duration_seconds;diagnostic.play_found=Boolean(audioPlayControl(node));diagnostic.download_found=Boolean(audioDownloadButton(node));diagnostic.url_found=Boolean(audioSource(node));
      if(menuProbeDone){diagnostic.status="aguardando_hover";diagnostic.reason="Versão de validação: somente o primeiro áudio localizado abre o menu automaticamente.";found.set(key,{...target,audioMeta:publicAudioMeta(audioMeta(key,{durationSeconds:playerDuration,extractionStatus:"pending",transcriptionStatus:"pending"})),audioTranscribed:false,hasVoiceMessage:true,type:"Áudio"});continue;}
      ...
      found.set(key,{...target,audioMeta:publicAudioMeta(audioMeta(key,{durationSeconds:playerDuration,extractionStatus:"pending",transcriptionStatus:"pending",error:diagnostic.reason||""})),audioTranscribed:false,hasVoiceMessage:true,type:"Áudio"});
    }
  };
