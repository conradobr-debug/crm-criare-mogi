...
const {cleanText, normalizedUiText, messageHash, continuationPrefix, playerDurationSeconds} = CaptureCore;
...
async function recoverAudioEntries(request={}){
  ...
  const targets=Array.isArray(request.audioTargets)?request.audioTargets:[];const targetKey=(target,index)=>target.id||target.message_id||`legacy:${index}`;const validDuration=value=>Number.isFinite(Number(value))&&Number(value)>0&&Number(value)<600?Number(value):null;const found=new Map();let menuProbeDone=false;const diagnostics=new Map(targets.map((target,index)=>{const key=targetKey(target,index);return [key,{message_id:key,position:target.chronological_position??index,sender:target.sender||"",date:target.date||"",message_time:target.time||"",duration_text:target.duration_text||"",duration_seconds:validDuration(target.duration),found_in_dom:false,bubble_rect:null,hover_target:[],hover_coordinates:null,pointer_events_sent:[],mutation_detected:false,synthetic_hover_failed:false,debugger_attached:false,mouse_move_dispatched:false,target_coordinates:null,trigger_found:false,trigger_visible:false,trigger_selector:"",trigger_clicked:false,dialog_found:false,menu_found:false,menu_render_location:"",context_menu_found:false,context_menu_opened:false,download_option_found:false,download_option_selector:"",download_clicked:false,download_id:null,download_state:"",filename:"",play_found:false,download_found:false,download_completed:false,url_found:false,file_obtained:false,mime_type:"",size:0,transcriber_called:false,debugger_detached:false,status:"aguardando_hover",reason:""}];}));
  const inspect=async()=>{
    for(const [position,node] of voiceMessageNodes(main).entries()){
      const detail=node.querySelector("[data-pre-plain-text]");const prefix=cleanText(detail?.getAttribute("data-pre-plain-text"))||"";const parts=prefixParts(prefix);const direction=messageDirection(node);const id=audioEntryId(node,prefix,position);const structuralTargets=targets.filter((item,index)=>{const key=targetKey(item,index);if(found.has(key))return false;const sameSender=!item.sender||item.sender===parts.sender;const sameDate=!item.date||item.date===parts.date;const sameTime=!item.time||item.time===parts.time;const sameDirection=!item.direction||item.direction===direction;return sameSender&&sameDate&&sameTime&&sameDirection;}).sort((a,b)=>Number(a.chronological_position||0)-Number(b.chronological_position||0));const target=targets.find((item,index)=>targetKey(item,index)===id)||structuralTargets[0];if(!target)continue;
      ...
      const recovered={...target,id:target.id||key,message_id:target.message_id||messageId(node)||null,sender:parts.sender||target.sender||null,direction,date:parts.date||target.date||null,time:parts.time||target.time||null,duration:playerDuration,duration_text:durationText,duration_seconds:playerDuration,duration_source:playerDuration?"whatsapp_player":"missing",duration_valid:Boolean(playerDuration),chronological_position:target.chronological_position??position,audioMeta:publicAudioMeta(audioMeta(key,{...(target.audioMeta||{}),durationSeconds:playerDuration,extractionStatus:target.audioMeta?.extractionStatus||"pending",transcriptionStatus:target.audioMeta?.transcriptionStatus||"pending"})),audioTranscribed:Boolean(target.audioTranscribed),hasVoiceMessage:true,type:"Áudio"};
      if(request.metadataOnly){diagnostic.status=playerDuration?"metadados_atualizados":"duracao_nao_lida";diagnostic.reason=playerDuration?"Duração lida diretamente do texto interno do player.":"O player foi localizado, mas não exibiu duração interna válida.";found.set(key,recovered);continue;}
      if(menuProbeDone){diagnostic.status="aguardando_hover";diagnostic.reason="Versão de validação: somente o primeiro áudio localizado abre o menu automaticamente.";found.set(key,recovered);continue;}
      menuProbeDone=true;const menu=await openMessageContextMenu(node,diagnostic);diagnostic.context_menu_found=Boolean(diagnostic.trigger_found);diagnostic.context_menu_opened=Boolean(menu);if(menu){const option=downloadMenuOption(menu);diagnostic.download_option_found=Boolean(option);diagnostic.download_option_selector=option?'button[role="menuitem"][aria-label="Baixar"]':"";diagnostic.status=option?"baixar_encontrado":"baixar_nao_encontrado";if(!option)diagnostic.reason="O menu abriu, mas não apresentou a ação Baixar.";closeContextMenu();}
      found.set(key,{...recovered,audioMeta:{...recovered.audioMeta,error:diagnostic.reason||""}});
    }
  };
