...
async function recoverAudioEntries(request={}){
  ...
  const inspect=async()=>{
    for(const [position,node] of voiceMessageNodes(main).entries()){
      const detail=node.querySelector("[data-pre-plain-text]");const prefix=cleanText(detail?.getAttribute("data-pre-plain-text"))||"";const parts=prefixParts(prefix);const detectedDirection=messageDirection(node);const id=audioEntryId(node,prefix,position);const exactTarget=targets.find((item,index)=>targetKey(item,index)===id||(item.message_id&&`wa:${item.message_id}`===id));if(!exactTarget)continue;const target=exactTarget;const direction=canonicalAudioDirection(parts.sender||target.sender,detectedDirection);
