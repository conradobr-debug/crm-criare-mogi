...
function canonicalMessageId(value){return cleanText(value||"").replace(/^(?:wa:)+/i,"");}
...
async function recoverAudioEntries(request={}){
  ...
  const targets=Array.isArray(request.audioTargets)?request.audioTargets:[];const targetKey=(target,index)=>target.id||target.message_id||`legacy:${index}`;const targetMessageId=target=>canonicalMessageId(target?.message_id||target?.id);const validDuration=value=>Number.isFinite(Number(value))&&Number(value)>0&&Number(value)<600?Number(value):null;const found=new Map();let menuProbeDone=false;const diagnostics=new Map(targets.map((target,index)=>{const key=targetKey(target,index);return [key,{message_id:targetMessageId(target)||key,position:target.chronological_position??index,sender:target.sender||"",date:target.date||"",message_time:target.message_time||target.time||"",duration_text:target.duration_text||"",duration_seconds:validDuration(target.duration_seconds||target.duration),found_in_dom:false,bubble_rect:null,hover_target:[],hover_coordinates:null,pointer_events_sent:[],mutation_detected:false,synthetic_hover_failed:false,debugger_attached:false,mouse_move_dispatched:false,target_coordinates:null,trigger_found:false,trigger_visible:false,trigger_selector:"",trigger_clicked:false,dialog_found:false,menu_found:false,menu_render_location:"",context_menu_found:false,context_menu_opened:false,download_option_found:false,download_option_selector:"",download_clicked:false,download_id:null,download_state:"",filename:"",play_found:false,download_found:false,download_completed:false,url_found:false,file_obtained:false,mime_type:"",size:0,transcriber_called:false,debugger_detached:false,status:"aguardando_hover",reason:""}];}));
  const inspect=async()=>{
    for(const [position,node] of voiceMessageNodes(main).entries()){
      const detail=node.querySelector("[data-pre-plain-text]");const prefix=cleanText(detail?.getAttribute("data-pre-plain-text"))||"";const parts=prefixParts(prefix);const detectedDirection=messageDirection(node);const domMessageId=canonicalMessageId(messageId(node));if(!domMessageId)continue;const exactTarget=targets.find(item=>targetMessageId(item)===domMessageId);if(!exactTarget)continue;const target=exactTarget;const direction=canonicalAudioDirection(parts.sender||target.sender,detectedDirection);
