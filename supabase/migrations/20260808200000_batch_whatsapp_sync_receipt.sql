-- Aplica um retorno da extensão em uma única transação HTTP. Isso evita que
-- lotes de verificação façam dezenas de requests concorrentes no plano gratuito.
  ...
  loop
    ...
    where record.workspace_id = p_workspace_id
      and (
        ...
        or (length(v_phone) between 8 and 15 and regexp_replace(coalesce(record.phone,''),'[^0-9]','','g') = v_phone)
      )
    ...
    insert into public.crm_whatsapp_lead_candidates(
      ...
      p_workspace_id, v_external, nullif(v_candidate->>'contact_wa_id',''),
      nullif(v_candidate->>'phone_e164',''), nullif(v_candidate->>'display_name',''),
      nullif(v_candidate->>'last_message_id',''), nullif(v_candidate->>'last_message_at','')::timestamptz,
      ...
      status = case when v_match is not null then 'known' else crm_whatsapp_lead_candidates.status end;
